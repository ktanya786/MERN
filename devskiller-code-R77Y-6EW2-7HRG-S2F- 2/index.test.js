...
const contentByFilename = Object.fromEntries(
  Array(60)
    .fill()
    .map((e, i) => [
      `https://server.com/file/${i}`,
      { success: true, payload: "example" },
    ])
);
            ...
            (x) =>
              new Promise((resolve, reject) =>
                setTimeout(() => {
                  ...
                  if (downloading)
                    reject(Error("connection in use by another download"));
...
const getReturnValues = (mockFn) =>
  mockFn.mock.results
    .filter((connection) => connection.type === "return")
...
describe("pooledDownload post-run analysis:", () => {
  ...
  it("all files downloaded", async () => {
    ...
    filenames.forEach((filename) =>
      expect(downloadCalls.some(([arg]) => arg === filename))
    );
    filenames.forEach((filename) =>
      expect(downloadCalls.some(([arg]) => arg === filename))
    );
  ...
  it("all files saved", async () => {
    ...
    contents.forEach((content) =>
      expect(save.mock.calls.some(([arg]) => arg === content))
    );
  });
  ...
  it("files were downloaded concurrently", async () => {
    connections.forEach((connection) => {
      ...
      expect(returnCount).toBeGreaterThanOrEqual(
        filenames.length / maxConcurrency - maxConcurrency
      );
      expect(returnCount).toBeLessThanOrEqual(
        filenames.length / maxConcurrency + maxConcurrency
      );
    });
  });
  ...
  it("downloads were distributed evenly among connections", async () => {
    connections.forEach((connection) => {
      const returnCount = getReturnValues(connection.download).length;
      expect(returnCount).toBeGreaterThanOrEqual(
        Math.floor(filenames.length / maxConcurrency)
      );
      expect(returnCount).toBeLessThanOrEqual(
        Math.ceil(filenames.length / maxConcurrency)
      );
    });
  });
});
...
describe("pooledDownload connection error handling:", () => {
    ...
    connect.mockImplementation(
      () =>
        new Promise((_, reject) =>
          setTimeout(() => reject(Error("server already at capacity")))
        )
    );
  });
  ...
  it('throws an error with message "connection failed" if no connection could be made', async () => {
    await expect(
      pooledDownload(connect, save, filenames, maxConcurrency)
...
describe("pooledDownload connection limit handling:", () => {
  ...
  it("downloads were distributed evenly among the successful connections", async () => {
    connections.forEach((connection) => {
      const returnCount = getReturnValues(connection.download).length;
      expect(returnCount).toBeGreaterThanOrEqual(
        Math.floor(filenames.length / crashAfterConnections)
      );
      expect(returnCount).toBeLessThanOrEqual(
        Math.ceil(filenames.length / crashAfterConnections)
      );
    });
  });
